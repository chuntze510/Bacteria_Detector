...
class YOLO(object):
    _defaults = {
        "model_path": 'model_data/ep747-loss27.397-val_loss23.897.h5', #指定YOLO訓練完成權重檔路徑及名稱
        "anchors_path": 'model_data/yolo_anchors.txt', #指定錨點定義檔路徑及名稱
        "classes_path": 'model_data/my_classes.txt', #指定自定義標籤檔路徑及名稱
        "score" : 0.01, #最低置信度門檻(0.01~0.99)
        "iou" : 0.3, #重疊區比例(0.01~1.0)
...
if __name__ == '__main__':
    t0 = timer()
    yolo=YOLO() #進行YOLO初始化
    path = 'dataset/JPEGImages/0_741.jpg' #指定待測影像檔案路徑及名稱
    try:
        t1 = timer()
